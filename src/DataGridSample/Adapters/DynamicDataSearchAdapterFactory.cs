...
namespace DataGridSample.Adapters
{
    ...
    /// </summary>
    public sealed class DynamicDataSearchAdapterFactory : IDataGridSearchAdapterFactory
    {
        private readonly Action<string> _log;

        public DynamicDataSearchAdapterFactory(Action<string> log)
        {
            _log = log;
            SearchPredicate = static _ => true;
        }

        public Func<Deployment, bool> SearchPredicate { get; private set; }
        ...
        private static Func<Deployment, bool> BuildPredicate(IReadOnlyList<SearchDescriptor> descriptors)
        {
            ...
            var compiled = new List<Func<Deployment, bool>>();
        ...
        private static Func<Deployment, bool>? Compile(SearchDescriptor descriptor)
        {
            if (descriptor == null)
            {
                return null;
        ...
        private static List<ColumnSelector> SelectColumns(SearchDescriptor descriptor)
        {
            ...
            foreach (var id in descriptor.ColumnIds)
            {
                ...
                var column = Columns.FirstOrDefault(c => string.Equals(c.Id, path, StringComparison.Ordinal));
                if (column != null)
                {
                    selected.Add(column);
                }
        ...
        {
            new("Service", d => d.Service),
            new("Region", d => d.Region),
            new("Ring", d => d.Ring),
            new("Status", d => d.Status),
            new("Started", d => d.Started.ToString("u", CultureInfo.InvariantCulture)),
            new("ErrorRate", d => d.ErrorRate.ToString("0.000", CultureInfo.InvariantCulture)),
            new("Incidents", d => d.Incidents.ToString(CultureInfo.InvariantCulture))
        };
        ...
        {
            public ColumnSelector(string id, Func<Deployment, string> getter)
            {
                Id = id;
                Getter = getter;
            }
            ...
            public Func<Deployment, string> Getter { get; }
        }
